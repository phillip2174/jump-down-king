...
export default class ObstacleAndItemView extends GameObjects.GameObject {
    ...
    private onCreate(obstacleCellView: ObstacleCellView) {
        obstacleCellView.onCreate(this.playerObject, this.bodyChecker, this.obstacleTriggerMove)
    }
    ...
    public SpawnObstacleLane(
        ...
        if (isSpawnMoveObstacle) {
            obstacleType =
                ObstacleType[
                    ObstacleType[
                        Math.floor(Math.random() * this.gamePlayPod.currentGameConfig.obstacleIndexArray.length)
                    ]
                ]
        } else {
